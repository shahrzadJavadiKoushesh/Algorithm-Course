    ...
    public class Grad {
        ...
        public static String[] lcsAlgorithm(String s1, String s2, int m, int p, int[] charWeights) {
            ...
            int i = minI;
            int j = minJ;
            ArrayList<String> lcsAlgo = new ArrayList<>();

            while (i > 0 && j > 0) {
                if (s1.charAt(i - 1) == s2.charAt(j - 1)) {
                    lcsAlgo.add(String.valueOf(s1.charAt(i - 1)));
                    i--;
                    j--;
            ...
            Collections.reverse(lcsAlgo);
            String lcsString = String.join("", lcsAlgo);

            return new String[]{String.valueOf(getTextWeight(s1, charWeights) + getTextWeight(s2, charWeights) - (maxCost * 2)), lcsString};
        }
    }
